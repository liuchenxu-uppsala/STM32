...
uint16_t send_packet[4];
uint16_t read_packet[4];
...
{
	static uint8_t local_recv_data_index = 0;
	static uint8_t current_status = 0;
	if(USART_GetITStatus(USART1, USART_IT_RXNE) == SET) {
		uint16_t temp_value = USART_ReceiveData(USART1);
		// Serial_Printf("temp_value:%d\n",temp_value);
		if (current_status == 0)
		{
			if(temp_value == 0xFF) {
				flag = 0;
			...
			read_packet[local_recv_data_index] = temp_value;
			local_recv_data_index++;
			if(local_recv_data_index > 3){
				current_status = 2;
			}
